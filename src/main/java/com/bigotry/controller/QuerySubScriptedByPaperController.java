...
/**
 * @author ：JJHu
 * @date ：Created in 2020/12/9 16:59
 * @description：
 * @modified By：
 * @version: $
 */
@FXMLController
public class QuerySubScriptedByPaperController implements Initializable {
    ...
    private Stage stage;
    @Autowired
    QuerySubScriptedByPaperView querySubScriptedByPaperView;
    @FXML
    private Label labelSaleNo;
    @FXML
    ...
    void init() {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                Parent parent = querySubScriptedByPaperView.getView();
                QuerySubScriptedByPaperController.this.stage = (Stage) parent.getScene().getWindow();
                stage.setWidth(707);
                stage.setHeight(760);
    ...
    public void handleQuerySubScriptedByPaper() {
        ...
        paperListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>(){
            ...
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                ...
                List<TbOrder> orders = queryMapper.queryAllOrderByPaperName(paperName);
                Integer subNo =0;
                Integer subMonth =0;
                if(!"".equals(paperName)){
                    if(orders!=null){//订单表中有记录
                        for(TbOrder order : orders){
                            subNo+=order.getSubscriptNo();
                            subMonth+=order.getSubscriptMonth();
                        }
                        Integer totalPrice = subNo*subMonth;
                        labelPaperName.setText(paperName);
                        labelSaleNo.setText(String.valueOf((subNo)));
                        labelTotalPrice.setText(totalPrice.toString());
                    }else{//订单表中无记录
                        labelPaperName.setText(paperName);
                        labelSaleNo.setText(String.valueOf(0));
