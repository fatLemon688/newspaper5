...
/**
 * @author ：JJHu
 * @date ：Created in 2020/12/8 15:17
 * @description：
 * @modified By：
 * @version: $
 */
@FXMLController
public class QueryByPersonController implements Initializable {
    ...
    private Stage stage;
    @Autowired
    QueryByPersonView queryByPersonView;
    @Autowired
    User user;
    @Autowired
    ...
    void init() {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                Parent parent = queryByPersonView.getView();
                QueryByPersonController.this.stage = (Stage) parent.getScene().getWindow();
                stage.setWidth(1125);
                stage.setHeight(705);
    ...
    public void handleQueryInfoByPerson() {
        ...
        //获取所有用户
        List<User> users  = queryMapper.queryAllUser();
        //遍历所有报刊并获取当前报刊的名称，并存入ListView
        for (User user:users) {
            items.add((user.getUserId()));
        }
        listView.setItems(items);
        //为ListView的选中项添加监听事件
        listView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>(){
            //ListView选中项的监听器类，一旦选中某项，就展示该用户的信息
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                String userId = (String) listView.getSelectionModel().getSelectedItem();
                if(!"".equals(userId)){
                    User userInfo = queryMapper.queryUserInfoById(userId);
                    if (userInfo != null) {
                        labelUserId.setText(userId);
                        labelUserName.setText(userInfo.getName());
                        labelIDCard.setText(userInfo.getIDCard());
                        labelPhone.setText(userInfo.getPhone());
                        labelAddress.setText(userInfo.getAddress());
                        labelDeptNo.setText(userInfo.getDeptNo().toString());
                    }
                }
            }
        });
    ...
    //按报刊查询基本信息
    @FXML
    public void handleQueryInfoByPaper(ActionEvent actionEvent) {
        stage.setWidth(1179);
        stage.setHeight(752);
        stage.setResizable(false);
