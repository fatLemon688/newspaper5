...
import com.bigotry.pojo.Department;
...
/**
 * @author ：JJHu
 * @date ：Created in 2020/12/9 21:18
 * @description：
 * @modified By：
 * @version: $
 */
@FXMLController
public class QuerySubScriptedByDeptController implements Initializable {
    ...
    private Stage stage;
    @Autowired
    QuerySubScriptedByDeptView querySubScriptedByDeptView;
    @FXML
    ...
    @FXML
    private Label labelSubNo;
    @FXML
    private Label labelTotalPrice;
    @FXML
    private Label labelPaperName;
    @FXML
    private Label labelSubMonthNo;
    @FXML
    private Label labelSubTime;
    @FXML
    ...
    void init() {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                Parent parent = querySubScriptedByDeptView.getView();
                QuerySubScriptedByDeptController.this.stage = (Stage) parent.getScene().getWindow();
                stage.setWidth(898);
                stage.setHeight(760);
    ...
    //按部门查询基本信息
    @FXML
    public void handleQueryInfoByDept() {
        stage.setWidth(727);
        stage.setHeight(760);
        stage.setResizable(false);
    ...
    //按报刊查询订阅情况
    @FXML
    public void handleQuerySubScriptedByPaper() {
        stage.setWidth(707);
        stage.setHeight(760);
        stage.setResizable(false);
    ...
    //按部门查询订阅情况
    @FXML
    public void handleQuerySubScriptedByDept() {
        //定义列表的数据
        ObservableList<String> deptItems = FXCollections.observableArrayList();
        //获取所有部门
        List<Department> depts  = queryMapper.queryAllDepartment();
        //遍历所有部门并获取当前部门的名称，并存入ListView
        for (Department dept:depts) {
            deptItems.add(dept.getDeptName());
        }
        deptListView.setItems(deptItems);
        //为ListView的选中项添加监听事件
        deptListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>(){
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                //ListView选中项的监听器类，一旦选中某项，就展示该报刊的内容，价格
                final String deptName = (String) deptListView.getSelectionModel().getSelectedItem();
                if(!"".equals(deptName)){//deptName不为空
                    Department deptInfo = queryMapper.queryDeptByDeptName(deptName);
                    if(deptInfo!=null){
                        Integer deptNo = deptInfo.getDeptNo();
                        //查询该部门下的所有订单
                        List<TbOrder> orders = queryMapper.queryCodeNameByDeptNo(deptNo.toString());
                        //定义列表的数据
                        ObservableList<String> orderItems = FXCollections.observableArrayList();
                        //遍历所有订单并获取当前订单的订单编号，并存入ListView
                        for (TbOrder order:orders) {
                            orderItems.add(order.getOrderNo());
                        }
                ...
                orderListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>(){
                    ...
                    public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                        ...
                        if(!"".equals(orderNo)){
                            ...
                            if(orderInfo!=null){//订单信息不为空
                                String timeStamp = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(Long.parseLong(orderNo.substring(0,13))));
                                labelSubTime.setText(timeStamp);
                                label1.setText("订阅了");
                                label2.setText("一共订阅了");
                                label3.setText("份");
                                label4.setText("订阅月数为：");
                                label5.setText("所需金额为(元):");
                                label6.setText("订阅时间:");
                                labelDeptName.setText(deptName);
                                labelPaperName.setText(queryMapper.queryPaperNameByOrderNo(orderNo).getPaperName());
                                labelTotalPrice.setText(orderInfo.getTotalPrice().toString());
                                labelSubNo.setText(orderInfo.getSubscriptNo().toString());
                                labelSubMonthNo.setText(orderInfo.getSubscriptMonth().toString());
                            }
                        }else{
                            //清空订单信息
                            clearOrderInfo();
