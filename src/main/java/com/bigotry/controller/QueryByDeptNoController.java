 ...
 */
@FXMLController
public class QueryByDeptNoController implements Initializable {
    ...
    private Stage stage;
    @Autowired
    QueryByDeptNoView queryByDeptNoView;
    @Autowired
    queryMapper queryMapper;
    @FXML
    private ListView deptNoListView;
    @Autowired
    User user;
    @Autowired
    NewsPaper newsPaper;
    ...
    void init() {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                Parent parent = queryByDeptNoView.getView();
                QueryByDeptNoController.this.stage = (Stage) parent.getScene().getWindow();
                stage.setWidth(727);
                stage.setHeight(760);
    ...
    public void handleQueryInfoByDept() {
        ...
        for (Department dept:depts) {
            deptItems.add(dept.getDeptNo().toString());
        }
        deptNoListView.setItems(deptItems);
        //为ListView的选中项添加监听事件
        deptNoListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>(){
            //ListView选中项的监听器类，一旦选中某项，就展示该报刊的内容，价格
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                String deptNo = (String) deptNoListView.getSelectionModel().getSelectedItem();
                List<User> users = queryMapper.queryAllUserByDeptNo(deptNo);
                Department deptInfo = queryMapper.queryDeptByDeptNo(deptNo);
                if(deptInfo!=null){
                    labelDeptName.setText(deptInfo.getDeptName());
                }
                ObservableList<String> userItems = FXCollections.observableArrayList();
                //遍历所有报刊并获取当前用户的名称，并存入ListView
                for (User user:users) {
                    userItems.add("ID: "+user.getUserId() + "   姓名： "+user.getName());
                }
