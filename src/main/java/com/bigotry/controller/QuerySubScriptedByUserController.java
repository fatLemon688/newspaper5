...
/**
 * @author ：JJHu
 * @date ：Created in 2020/12/8 23:47
 * @description：
 * @modified By：
 * @version: $
 */
@FXMLController
public class QuerySubScriptedByUserController implements Initializable {
    ...
    private Stage stage;
    @Autowired
    QuerySubScriptedByUserView querySubScriptedByUserView;
    @FXML
    ...
    void init() {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                Parent parent = querySubScriptedByUserView.getView();
                QuerySubScriptedByUserController.this.stage = (Stage) parent.getScene().getWindow();
                stage.setWidth(999);
                stage.setHeight(760);
    ...
    //按用户查询订阅情况
    @FXML
    public void handleQuerySubScriptedByUser() {
        //定义列表的数据
        final ObservableList<String> userItems = FXCollections.observableArrayList();
        //获取所有有订阅报刊的用户信息
        final List<User>  users= queryMapper.querySubScriptedUser();
        //遍历所有用户并获取当前用户的名称，并存入ListView
        for (User user:users) {
            userItems.add(user.getUserId());
        }
        userListView.setItems(userItems);
        Integer userCount = users.size();
        labelUserCount.setText(userCount.toString());
        //为ListView的选中项添加监听事件
        userListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>(){
            ...
            public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                String userId = (String) userListView.getSelectionModel().getSelectedItem();
                //查询当前用户的所有订单
                List<TbOrder> orders = queryMapper.queryAllOrderByUserId(userId);
                ObservableList<String> orderItems = FXCollections.observableArrayList();
                final User userInfo = queryMapper.queryUserInfoById(userId);
                if(userInfo!=null){
                    labelUserName.setText(userInfo.getName());
                }
                ...
                orderListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<String>(){
                    //ListView选中项的监听器类，一旦选中某项，就展示该订单的信息
                    @Override
                    public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
                        ...
                        if(orderInfo!=null){
                            labelUserName1.setText(userInfo.getName());
